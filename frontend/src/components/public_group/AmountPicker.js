...
import calculateFees from '../../lib/calculate_fees';
...
export default class AmountPicker extends React.Component {
  render() {
    ...
    var stripeDescription =  `${formatCurrency(amount, currency, { compact: false })} ${frequencyHuman}`;
    ...
    if (group.feesOnTop) {
      fees = calculateFees(amount, group.hostFeePercent);
      stripeDescription = `${formatCurrency(amount, currency, { compact: false })} (+ ${formatCurrency(fees, currency, { compact: true })} fees) ${frequencyHuman}`;
    }

    const chargeAmount = amount + fees;
    return (
      <div>
        {tier.presets && (
          <DonationPicker
        ...
        <div className='max-width-1 mx-auto' style={{position: 'absolute', left: '0', right: '0', margin: '-25px auto'}}>
          ...
          {hasStripe && !hasPaypal && ( // paypal has priority -> to refactor after prototyping phase
            <StripeCheckout
              token={(token) => onToken({amount:chargeAmount, frequency, currency, token})}
              stripeKey={stripeKey}
              name={group.name}
              currency={currency}
              amount={convertToCents(chargeAmount)}
          ...
          <div className='PublicGroupForm-disclaimer h6 mt2' style={{minHeight: 38}}>
            {i18n.getString('disclaimer')} {group.host.name} {`${formatCurrency(chargeAmount, currency, { compact: false })}`} {frequencyHuman} {i18n.getString('for')} {group.name}. {cancellationDisclaimer}
          </div>
        </div>
      </div>
    );
